...
int main(int argc, char **argv) {
  ...
  ros::Publisher odom_global = nh.advertise<nav_msgs::Odometry>("odom_global_003",100);
  ...
  ros::Subscriber mission=nh.subscribe<geometry_msgs::InertiaStamped>("/fcu_bridge/mission_003", 100, missionHandler);
  ros::Publisher path_global = nh.advertise<nav_msgs::Path>("/path_global_003", 100);
        ...
        if (mavlink_parse_char(chan, buffer[i], &msg_received, &status)){
            ...
            switch (msg_received.msgid) {
              case MAVLINK_MSG_ID_HEARTBEAT:
                printf("003 Received heartbeat time: %f\n", ros::Time::now().toSec());
