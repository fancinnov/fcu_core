...
int main(int argc, char **argv) {
  ...
  ros::Publisher odom_global = nh.advertise<nav_msgs::Odometry>("odom_global_001",100);
  ...
  ros::Publisher path_global = nh.advertise<nav_msgs::Path>("/path_global_001", 100);
        ...
        if (mavlink_parse_char(chan, buffer[i], &msg_received, &status)){
            ...
            switch (msg_received.msgid) {
              case MAVLINK_MSG_ID_HEARTBEAT:
                printf("001 Received heartbeat time: %f\n", ros::Time::now().toSec());
